...
def set_user_active(username: str, active: int):
    try:
        ...
        if int(active) != 1:
            # machine_id is retained as a legacy DB column but now stores the active session token.
...
def register_session(username: str):
    """Create the single authoritative session for an account.

    The legacy users.machine_id column is repurposed as the active session token.
    Every new login replaces this token, which immediately invalidates the previous browser/device.
    """
    username = (username or "").strip()
    ...
    try:
        # Authoritative takeover: this invalidates every previous session immediately.
        supabase_admin.table("users").update({"machine_id": session_id}).eq("username", username).execute()
    except Exception as e:
        raise RuntimeError("Could not establish a secure session. " + friendly_db_error(e)) from e

    st.session_state["session_id"] = session_id
    st.session_state["session_started_at"] = now
    st.session_state["last_activity_local"] = now

    # Registry/audit table is useful operationally, but enforcement lives in users.machine_id.
    try:
        supabase_admin.table("user_sessions").update({"active": 0}).eq("username", username).execute()
...
def validate_current_session():
    """Server-side single-session validation performed on every app rerun."""
    ...
    try:
        res = supabase_admin.table("users").select(
            "username,role,expires_at,active,machine_id"
        ).eq("username", username).execute()
    except Exception as e:
    ...
    active_token = str(row.get("machine_id") or "").strip()
    if not active_token or active_token != str(session_id):
        return False, "This account was signed in on another device. This session has been closed."

    # Keep role/expiry authoritative in case an admin changes them during a session.
    ...
    # Optional inactivity timeout using the remote session table when available.
    try:
        sres = supabase_admin.table("user_sessions").select("last_seen,active").eq(
            "session_id", session_id
        ).execute()
        if sres.data:
            srow = sres.data[0]
            ...
            if last_seen and utc_now() - last_seen > timedelta(minutes=SESSION_IDLE_MINUTES):
                terminate_session(username, session_id=session_id)
                return False, f"Session expired after {SESSION_IDLE_MINUTES} minutes of inactivity."
    except Exception:
        # Single-session security still remains enforced by users.machine_id.
        pass
...
def terminate_session(username: str, session_id: str | None = None):
    """Terminate an account session without accidentally killing a newer takeover session."""
    username = (username or "").strip()
    try:
        q = supabase_admin.table("users").update({"machine_id": None}).eq("username", username)
        if session_id:
            q = q.eq("machine_id", session_id)
        q.execute()
    except Exception as e:
        raise RuntimeError(friendly_db_error(e)) from e

    try:
        q = supabase_admin.table("user_sessions").update({"active": 0}).eq("username", username)
        if session_id:
...
def admin_panel():
    ...
    with st.sidebar.expander("🩺 System diagnostics"):
        ...
        try:
            supabase_admin.table("users").select("machine_id").limit(1).execute()
            st.success("Single-session token field: available")
        except Exception as e:
            st.error("Single-session token field is unavailable. Check the users.machine_id column.")
