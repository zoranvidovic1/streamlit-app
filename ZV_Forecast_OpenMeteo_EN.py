    ...
    try:
        ...
        if not res.data:
            if not admin_password:
                st.error("Initial setup: add DEFAULT_ADMIN_PASSWORD to Streamlit secrets.")
                st.code('DEFAULT_ADMIN_USERNAME = "AdminZija"\nDEFAULT_ADMIN_PASSWORD = "enter-a-strong-password"')
                st.stop()
            create_user(admin_username, admin_password, 3650, role="admin")

    except Exception as e:
        st.error("Supabase error while checking the admin user.")
        st.code(str(e))
        st.stop()
...
def register_failed_login(username: str, max_attempts: int = 5, lock_minutes: int = 15):
    ...
    try:
        ...
        st.warning("Login protection is temporarily unavailable, but the application will continue running.")
        st.code(str(e))
...
def admin_panel():
    ...
    with st.sidebar.expander("➕ Add user"):
        new_username = st.text_input("Username", placeholder="npr. client_01", key="new_username")
        new_password = st.text_input("Temporary password", type="password", placeholder="Minimum 10 znakova", key="new_password")
        days_valid = st.number_input("Access duration (days)", min_value=1, max_value=3650, value=30)
        ...
        if st.button("Create user", use_container_width=True, type="primary"):
            try:
                ...
                st.success(f"User '{new_username}' was created.")
            except Exception as e:
                st.error(f"Error: {e}")
    ...
    with st.sidebar.expander("⏳ Extend / activate user"):
        users_df = get_users_df()
        if not users_df.empty:
            username = st.selectbox("User", users_df["username"].tolist(), key="extend_user")
            extra_days = st.number_input("Add days", 1, 3650, 30, key="extra_days")
            if st.button("Extend access", use_container_width=True):
                try:
                    extend_user(username, extra_days)
                    st.success("Access extended.")
                except Exception as e:
                    st.error(str(e))
    ...
    with st.sidebar.expander("🛑 Activate / deactivate"):
        users_df = get_users_df()
        if not users_df.empty:
            username = st.selectbox("User", users_df["username"].tolist(), key="activate_user")
    ...
    with st.sidebar.expander("🔑 Change password"):
        users_df = get_users_df()
        if not users_df.empty:
            username = st.selectbox("User", users_df["username"].tolist(), key="pass_user")
            new_pass = st.text_input("New password", type="password", key="pass_new")
    ...
    with st.sidebar.expander("💻 Reset user device"):
        users_df = get_users_df()
        if not users_df.empty:
            ...
            if st.button("Reset linked device", use_container_width=True):
                reset_user_machine(username)
                st.success("The linked device was reset. The user can now sign in from a new device.")
    ...
    with st.sidebar.expander("🧾 Login audit"):
        try:
            ...
            st.warning("Could not load login audit.")
    with st.sidebar.expander("🟢 Active sessions"):
        try:
            ...
            if not sess_df.empty:
                ...
                if st.button("Disconnect active sessions", use_container_width=True):
                    supabase.table("user_sessions").update({
                        "active": 0
                    }).eq("username", kill_user).execute()

                    st.success(f"User {kill_user} was disconnected.")
                    st.rerun()
            else:
                st.info("No active sessions.")

        except Exception as e:
            st.warning("Could not load active sessions.")
            st.code(str(e))
...
def login_page():
    ...
    with center:
        ...
        with st.form("login_form", clear_on_submit=False):
            ...
            st.caption("Access is protected by session control and login auditing.")
        ...
        if submitted:
            ...
            if locked:
                st.error(f"Account is temporarily locked until: {str(locked_until)[:19]}")
            ...
            if success:
                ...
                if not session_id:
                    st.error("Could not create a session. Please try again.")
                    return
