...
try:
    ...
    # Server-side admin key: prefer the current Supabase secret key,
    # then fall back to the legacy service_role key.
    _secret_key = get_secret_safe("SUPABASE_SECRET_KEY", "")
    _service_key = get_secret_safe("SUPABASE_SERVICE_ROLE_KEY", "")
    _admin_key = _secret_key or _service_key
    supabase_admin = create_client(SUPABASE_URL, _admin_key) if _admin_key else supabase
    SUPABASE_ADMIN_MODE = "secret" if _secret_key else ("service_role" if _service_key else "public-key fallback")
except Exception:
...
def create_user(username: str, password: str, days_valid: int, role: str = "user"):
    ...
    try:
        ...
        if SUPABASE_ADMIN_MODE == "public-key fallback":
            raise RuntimeError(
                "Admin write access is not configured. Add SUPABASE_SECRET_KEY "
                "(recommended) or SUPABASE_SERVICE_ROLE_KEY to Streamlit Secrets. "
                "Without a server-side admin key, creating users may be blocked by Supabase RLS."
            )
        ...
        payload = {
            ...
            "active": 1,
            "machine_id": None,  # legacy field; no longer used for session enforcement
        ...
        # Never report success unless the row is actually visible in the database.
        verify = supabase_admin.table("users").select(
            "id,username,role,active,created_at,expires_at"
        ).eq("username", username).execute()
        if not verify.data:
            raise RuntimeError(
                "Supabase did not persist the new user. No user was created. "
                "Check the server-side secret key, table grants, and RLS configuration."
            )
...
def get_users_df():
    try:
        res = supabase_admin.table("users").select(
            "id, username, role, created_at, expires_at, active, active_session_id, last_login_at, last_login_ip, last_browser"
...
def set_user_active(username: str, active: int):
    try:
        ...
        supabase_admin.table("users").update(payload).eq("username", username).execute()
        if int(active) != 1:
            try:
                supabase_admin.table("user_sessions").update({
                    "active": 0,
                    "ended_at": utc_iso(),
                    "logout_reason": "account_disabled",
                }).eq("username", username).eq("active", 1).execute()
            except Exception:
                pass
    except Exception as e:
        raise RuntimeError(friendly_db_error(e)) from e
...
def change_user_password(username: str, new_password: str):
    ...
    if not any(c.isalpha() for c in new_password) or not any(c.isdigit() for c in new_password):
        raise ValueError("Password must contain at least one letter and one number.")
    try:
        # Changing a password immediately terminates every existing session for this account.
...
SESSION_IDLE_MINUTES = int(get_secret_safe("SESSION_IDLE_MINUTES", "60") or 60)


def register_session(username: str):
    """Create the only valid session for an account.

    Enforcement lives in users.active_session_id. Every successful new login
    atomically replaces that value from the application's point of view and
    invalidates the previous browser/session on its next server-side check.
    user_sessions remains the operational history and audit registry.
    """
    username = (username or "").strip()
    ...
    # 1) Close previous active session records, preserving history.
    try:
        supabase_admin.table("user_sessions").update({
            "active": 0,
            "ended_at": now,
    ...
    # 2) Make this session authoritative and store last-login metadata.
    try:
        supabase_admin.table("users").update({
            "active_session_id": session_id,
            "last_login_at": now,
            "last_login_ip": ip_address,
            "last_browser": browser_info,
        }).eq("username", username).execute()
    except Exception as e:
        raise RuntimeError(
            "Could not establish a secure session. Make sure the Supabase migration was executed. "
            + friendly_db_error(e)
        ) from e

    # 3) Store the new session in history/monitoring.
    try:
    ...
    st.session_state["last_activity_local"] = now
    return session_id


def validate_current_session():
    """Validate account status, expiry, and single-session ownership on every rerun."""
    user = st.session_state.get("user") or {}
    username = (user.get("username") or "").strip()
    session_id = st.session_state.get("session_id")
    if not username or not session_id:
        return False, "Your session is no longer valid. Please sign in again."

    try:
        res = supabase_admin.table("users").select(
            "username,role,expires_at,active,active_session_id"
        ).eq("username", username).execute()
    except Exception as e:
        return False, "Session validation failed: " + friendly_db_error(e)
    ...
    if int(row.get("active", 0) or 0) != 1:
        return False, "This account has been deactivated by the administrator."

    exp = parse_utc_datetime(row.get("expires_at"))
    if exp and utc_now() > exp:
        try:
            terminate_session(username, session_id=session_id, reason="account_expired")
        except Exception:
            pass
        return False, "This account has expired. Please contact the administrator."

    active_session_id = str(row.get("active_session_id") or "").strip()
    if not active_session_id or active_session_id != str(session_id):
        return False, "This account was signed in on another device. This session has been closed."
    ...
    # Optional operational registry and inactivity timeout.
    try:
        sres = supabase_admin.table("user_sessions").select(
            "last_seen,active"
        ).eq("session_id", session_id).execute()
        if sres.data:
            srow = sres.data[0]
            if int(srow.get("active", 0) or 0) != 1:
                return False, "This session was terminated by the administrator."
            last_seen = parse_utc_datetime(srow.get("last_seen"))
            if last_seen and utc_now() - last_seen > timedelta(minutes=SESSION_IDLE_MINUTES):
                terminate_session(username, session_id=session_id, reason="inactivity_timeout")
                return False, f"Session expired after {SESSION_IDLE_MINUTES} minutes of inactivity."
    except Exception:
        # Single-session enforcement still remains authoritative via users.active_session_id.
        pass
...
def touch_session():
    ...
    now = utc_iso()
    st.session_state["last_activity_local"] = now
    if st.session_state.get("remote_session_registered"):
        try:
            supabase_admin.table("user_sessions").update({"last_seen": now}).eq(
                "session_id", session_id
            ).eq("active", 1).execute()
        except Exception:
            st.session_state["remote_session_registered"] = False
...
def terminate_session(username: str, session_id: str | None = None, reason: str = "admin_force_logout"):
    """Terminate a session without allowing an old browser to kill a newer takeover session."""
    username = (username or "").strip()
    now = utc_iso()

    # Clear users.active_session_id only when it still points to the session being terminated.
    try:
        q = supabase_admin.table("users").update({"active_session_id": None}).eq("username", username)
        if session_id:
            q = q.eq("active_session_id", session_id)
        q.execute()
    except Exception as e:
        raise RuntimeError(friendly_db_error(e)) from e

    try:
        q = supabase_admin.table("user_sessions").update({
            "active": 0,
            "ended_at": now,
            "logout_reason": reason,
        }).eq("username", username).eq("active", 1)
        if session_id:
...
def clear_local_auth_state():
    for key in [
        "logged_in", "user", "session_id", "remote_session_registered",
        "session_started_at", "last_activity_local"
    ]:
...
def logout_user():
    ...
    if username and session_id:
        try:
            terminate_session(username, session_id=session_id, reason="user_logout")
        except Exception:
            pass
...
def admin_panel():
    ...
    with st.sidebar.expander("➕ Add user", expanded=True):
        ...
        new_password = st.text_input("Temporary password", type="password", placeholder="At least 12 characters · letters + numbers", key="new_password")
        ...
        if st.button("Create user", use_container_width=True, type="primary", key="create_user_btn"):
            try:
                created = create_user(new_username, new_password, int(days_valid), role=new_role)
                st.success(
                    f"User '{created.get('username', new_username.strip())}' was created and verified in Supabase."
                )
            except Exception as e:
                st.error(str(e))
        ...
        try:
            ...
            if users_df.empty:
                ...
                view_cols = [c for c in [
                    "username", "role", "status", "last_login_at", "last_login_ip", "last_browser", "expires_at", "days_left"
                ] if c in users_df.columns]
                view = users_df[view_cols].copy()
                if "expires_at" in view.columns:
                    view["expires_at"] = view["expires_at"].dt.strftime("%Y-%m-%d")
                if "last_login_at" in view.columns:
                    view["last_login_at"] = view["last_login_at"].dt.strftime("%Y-%m-%d %H:%M UTC")
                if "last_browser" in view.columns:
                    view["last_browser"] = view["last_browser"].fillna("").astype(str).str.slice(0, 42)
                st.dataframe(view, use_container_width=True, height=280, hide_index=True)
        except Exception as e:
            st.error(str(e))
    ...
    with st.sidebar.expander("🔑 Change password"):
        if usernames:
            ...
            new_pass = st.text_input("New password", type="password", placeholder="At least 12 characters · letters + numbers", key="pass_new")
            if st.button("Change password", use_container_width=True, key="change_password_btn"):
                try:
                    change_user_password(username, new_pass)
                    st.success("Password changed. All active sessions for this user were terminated.")
                except Exception as e:
                    st.error(str(e))
        else:
            st.info("No users available.")
    ...
    with st.sidebar.expander("🟢 Active sessions"):
        try:
            ...
            if sess_df.empty:
                ...
                cols = [c for c in ["username", "ip_address", "browser_info", "created_at", "last_seen"] if c in sess_df.columns]
                active_view = sess_df[cols].copy()
                if "browser_info" in active_view.columns:
                    active_view["browser_info"] = active_view["browser_info"].fillna("").astype(str).str.slice(0, 50)
                st.dataframe(active_view, use_container_width=True, height=240, hide_index=True)
                ...
                if st.button("Disconnect session", use_container_width=True, key="disconnect_session_btn"):
                    terminate_session(kill_user, reason="admin_force_logout")
                    st.success(f"Session for {kill_user} was terminated. The user must sign in again.")
        except Exception:
            st.info("Remote session registry is optional and is not currently available.")

    with st.sidebar.expander("📚 Session history"):
        try:
            hist = supabase_admin.table("user_sessions").select("*").order("created_at", desc=True).limit(50).execute()
            hist_df = pd.DataFrame(hist.data or [])
            if hist_df.empty:
                st.info("No session history available.")
            else:
                cols = [c for c in ["username", "ip_address", "created_at", "last_seen", "ended_at", "logout_reason", "active"] if c in hist_df.columns]
                st.dataframe(hist_df[cols], use_container_width=True, height=260, hide_index=True)
        except Exception:
            st.info("Session history is not currently available.")

    with st.sidebar.expander("🩺 System diagnostics"):
        try:
            supabase_admin.table("users").select("id").limit(1).execute()
            st.caption(f"Admin DB mode: {SUPABASE_ADMIN_MODE}")
        ...
        st.caption(f"Session policy: one active session per account · idle timeout {SESSION_IDLE_MINUTES} min")
        try:
            supabase_admin.table("users").select("active_session_id,last_login_at,last_login_ip,last_browser").limit(1).execute()
            st.success("SaaS session fields: available")
        except Exception:
            st.error("SaaS session fields are missing. Run the supplied Supabase SQL migration.")
    ...
    with center:
        ...
        if submitted:
            ...
            try:
                ...
                clear_local_auth_state()
                log_login_attempt(username, False, "Secure session creation failed")
                st.error(str(e))
                return

            st.session_state["logged_in"] = True
            log_login_attempt(username, True, "Login success · previous session replaced")
            st.rerun()
...
def require_login():
    ...
    ok, reason = validate_current_session()
    if not ok:
        clear_local_auth_state()
        st.warning(reason)
        login_page()
...
# Automatic session guard: when supported by the installed Streamlit version,
# the old browser notices a login takeover even if the user does not click anything.
if hasattr(st, "fragment"):
    @st.fragment(run_every="10s")
    def _session_guard_fragment():
        if st.session_state.get("logged_in", False):
            ok, _reason = validate_current_session()
            if not ok:
                clear_local_auth_state()
                st.rerun()
else:
