...
if not SUPABASE_URL or not SUPABASE_KEY:
    st.error("Nedostaje Supabase konfiguracija.")
    st.info("Dodajte SUPABASE_URL i SUPABASE_KEY u Streamlit Cloud → Settings → Secrets.")
    st.stop()

try:
    supabase = create_client(SUPABASE_URL, SUPABASE_KEY)
except Exception:
    st.error("Povezivanje sa Supabase bazom nije uspelo.")
    st.caption("Proverite SUPABASE_URL i SUPABASE_KEY u Streamlit Secrets.")
    st.stop()
...
def ensure_default_admin():
    """Create the first admin only from deployment secrets; never hard-code credentials."""
    try:
        admin_username = get_secret_safe("DEFAULT_ADMIN_USERNAME", "AdminZija").strip()
        admin_password = get_secret_safe("DEFAULT_ADMIN_PASSWORD", "").strip()
        res = supabase.table("users").select("id").eq("username", admin_username).execute()

        if not res.data:
            if not admin_password:
                st.error("Prvo podešavanje: dodaj DEFAULT_ADMIN_PASSWORD u Streamlit secrets.")
                st.code('DEFAULT_ADMIN_USERNAME = "AdminZija"\nDEFAULT_ADMIN_PASSWORD = "unesi-jaku-lozinku"')
                st.stop()
            create_user(admin_username, admin_password, 3650, role="admin")

    except Exception as e:
        st.error("Supabase greška pri proveri admin korisnika.")
        st.code(str(e))
        st.stop()
...
def admin_panel():
    ...
    st.sidebar.markdown(
        f"**👤 {user.get('username', '')}**  \n"
        f"`{role}` · pristup do {str(user.get('expires_at', ''))[:10]}"
    )
    ...
    st.sidebar.markdown("---")
    st.sidebar.subheader("🛡️ Administracija")

    with st.sidebar.expander("➕ Dodaj korisnika"):
        new_username = st.text_input("Korisničko ime", placeholder="npr. client_01", key="new_username")
        new_password = st.text_input("Privremena lozinka", type="password", placeholder="Minimum 10 znakova", key="new_password")
        days_valid = st.number_input("Trajanje pristupa (dani)", min_value=1, max_value=3650, value=30)
        role = st.selectbox("Uloga", ["user", "admin"])

        if st.button("Kreiraj korisnika", use_container_width=True, type="primary"):
            try:
                if len(new_username.strip()) < 3:
                    raise ValueError("Korisničko ime mora imati najmanje 3 znaka.")
                if len(new_password) < 10:
                    raise ValueError("Lozinka mora imati najmanje 10 znakova.")
...
def login_page():
    ...
    with center:
        ...
        <div class="zv-login-hero">
          ...
          <div class="zv-login-copy">Professional PV portfolio forecasting, weather intelligence and production analytics.</div>
        ...
        with st.form("login_form", clear_on_submit=False):
            ...
            submitted = st.form_submit_button("Prijavi se  →", use_container_width=True, type="primary")
            st.caption("Pristup je zaštićen kontrolom sesije i evidencijom prijava.")
        ...
        if submitted:
            ...
            if locked:
                st.error(f"Nalog je privremeno zaključan do: {str(locked_until)[:19]}")
            ...
            if success:
                ...
                if not session_id:
                    st.error("Greška pri kreiranju sesije. Pokušajte ponovo.")
                    return
...
def merge_weather_sources_commercial(sources, timezone):
    ...
    # Open-Meteo is the forecast backbone. Build a master timeline from every
    # available source so a shorter provider cannot truncate the forecast horizon.
    priority = ["open_meteo", "visual_crossing", "nasa_power"]
    base_key = next(k for k in priority if k in dfs)
    master_index = None
    for _df in dfs.values():
        master_index = _df.index if master_index is None else master_index.union(_df.index)
    master_index = master_index.sort_values()
    base = dfs[base_key].reindex(master_index).copy()
    ...
    def weighted_col(col, weights):
        ...
        for src, w in weights.items():
            if src in dfs and col in dfs[src].columns:
                ser = pd.to_numeric(dfs[src][col], errors="coerce").rename(src)
                weighted_parts.append(ser * float(w))
                weight_parts.append(ser.notna().astype(float) * float(w))
        if weighted_parts:
            numerator = pd.concat(weighted_parts, axis=1).sum(axis=1, min_count=1)
            denominator = pd.concat(weight_parts, axis=1).sum(axis=1)
    ...
    # Interpoliraj samo numeričke kolone — ne tekstualne source_* kolone
    num_cols = base.select_dtypes(include=[np.number]).columns
    other_cols = base.columns.difference(num_cols)

    base[num_cols] = base[num_cols].interpolate(method="time").ffill().bfill()

    # Tekstualne kolone samo popuni napred/nazad
    if len(other_cols) > 0:
        base[other_cols] = base[other_cols].ffill().bfill()
...
def fetch_nasa_power(latitude: float, longitude: float, timezone: str) -> pd.DataFrame:
    ...
    idx = pd.to_datetime(list(params["T2M"].keys()), format="%Y%m%d%H", errors="coerce", utc=True)
    df = pd.DataFrame(index=idx).loc[lambda x: ~x.index.isna()]
...
with st.sidebar:
    ...
    visual_crossing_key = get_secret_safe("VISUAL_CROSSING_KEY", "")
    solcast_key = ""   # Solcast trenutno nije aktivan; ostaje za buduću premium verziju
    ...
    if weather_provider == "Solcast future":
        st.info("Solcast trenutno nije aktivan. Aplikacija koristi Open-Meteo kao fallback bez API ključa.")

    res_min = st.selectbox(
        "Vremenska rezolucija (min)",
        [5, 10, 15, 30, 60],
    ...
    temp_model = st.selectbox("Temperaturni model (SAPM)", options=temp_models, index=default_idx)

    export_utc = st.checkbox("📦 Export u UTC", value=APP_DEFAULTS["EXPORT_UTC"])

    st.markdown("---")
    st.subheader("🏭 Portfolio elektrana")
    ...
    upl = st.file_uploader("Učitaj konfiguraciju (JSON)", type=["json"], help="Uvoz kompletne konfiguracije elektrana.")
    if upl is not None:
        try:
    ...
    with st.expander("➕ Dodaj novu elektranu", expanded=False):
    ...
    if st.button("🔄 Osveži forecast", use_container_width=True, type="primary"):
        st.cache_data.clear()
        st.rerun()
...
st.caption("Najvažniji pokazatelji za trenutno učitani portfolio i aktivni forecast horizont.")
