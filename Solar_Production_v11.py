...
def create_user(username: str, password: str, days_valid: int, role: str = "user"):
    ...
    if not username:
        raise ValueError("Username ne sme biti prazan.")
    if not password:
        raise ValueError("Lozinka ne sme biti prazna.")
    ...
    c.execute("""
        INSERT INTO users 
        (username, password_hash, role, created_at, expires_at, active)
        VALUES (?, ?, ?, ?, ?, ?)
    """, (
        username,
...
def ensure_default_admin():
    ...
    if count_admin == 0:
        create_user("admin", "admin123", 3650, role="admin")
...
def authenticate(username: str, password: str):
    ...
    c.execute("""
        SELECT username, password_hash, role, expires_at, active, machine_id
        FROM users
        WHERE username = ?
    """, (username.strip(),))
    ...
    db_username, password_hash, role, expires_at, active, machine_id = user
    ...
    # Prvo logovanje: veži korisnika za ovaj računar
    if machine_id is None or str(machine_id).strip() == "":
        c.execute(
            "UPDATE users SET machine_id = ? WHERE username = ?",
            (current_machine_id, db_username)
        )
        conn.commit()
        conn.close()
    ...
    # Ako pokušava sa drugog računara — blokiraj aplikaciju
    if machine_id != current_machine_id:
        conn.close()
        return False, (
            "Ovaj korisnički nalog je već aktiviran na drugom računaru. "
            "Pristup sa ovog uređaja nije dozvoljen."
        )
...
def get_users_df():
    ...
    df = pd.read_sql_query("""
        SELECT id, username, role, created_at, expires_at, active
        FROM users
        ORDER BY id DESC
    """, conn)
    conn.close()
...
def extend_user(username: str, extra_days: int):
    ...
    c.execute(
        "UPDATE users SET expires_at = ?, active = 1 WHERE username = ?",
...
def set_user_active(username: str, active: int):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("UPDATE users SET active = ? WHERE username = ?", (int(active), username))
    conn.commit()
    conn.close()
...
def login_page():
    ...
    st.markdown("""
    <div class="login-box">
        <div class="login-title">☀️ ZV Consulting Forecast</div>
        <div class="login-subtitle">Secure access to PV portfolio forecasting platform</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with st.sidebar.expander("➕ Dodaj korisnika"):
        ...
        if st.button("Kreiraj korisnika", use_container_width=True):
            try:
                ...
                st.error("Taj username već postoji.")
    ...
    with st.sidebar.expander("💻 Reset računara korisnika"):
        ...
        if not users_df.empty:
            ...
            if st.button("Resetuj vezani računar", use_container_width=True):
                ...
                c = conn.cursor()
                c.execute(
                    "UPDATE users SET machine_id = NULL WHERE username = ?",
                    (username,)
                )
                conn.commit()
                conn.close()
...
def fetch_weather(provider: str, latitude: float, longitude: float, timezone: str,
    ...
    if provider == "Solcast fallback":
        return fetch_solcast_placeholder(latitude, longitude, timezone, solcast_key)
...
with st.sidebar:
    ...
    weather_provider = st.selectbox(
        "Weather source",
        ["Open-Meteo", "Visual Crossing", "Solcast fallback"],
        index=0,
        help="Open-Meteo radi bez ključa. Visual Crossing traži API key. Solcast fallback je priprema za buduću integraciju."
    )

    visual_crossing_key = ""
    solcast_key = ""

    if weather_provider == "Visual Crossing":
        visual_crossing_key = st.text_input("Visual Crossing API key", type="password")

    if weather_provider == "Solcast fallback":
        solcast_key = st.text_input("Solcast API key", type="password")
        st.info("Trenutno fallback koristi Open-Meteo dok se Solcast endpoint ne poveže sa tvojim API nalogom.")
    ...
    json_text = st.text_area("List of plants (JSON)", key="plants_json", height=260)
    ...
    with tabs[i]:
        ...
        with sub2:
            ...
            with c1:
                ...
                st.plotly_chart(style_fig(f1, "Irradiance + clear-sky", height=340, ytitle="W/m²"), use_container_width=True)
...
st.caption("© 2026 ZV Consulting Forecast • Open-Meteo + pvlib • Streamlit SaaS prototype")