...
def authenticate(username: str, password: str):
    ...
    if machine_id is None or str(machine_id).strip() == "":
        ...
        return False, "Ovaj korisnički nalog je već aktiviran na drugom računaru."
...
def validate_active_session(timeout_minutes: int = 60):
    ...
    if sess.get("machine_id") != get_machine_id():
        return False
...
def register_failed_login(username: str, max_attempts: int = 5, lock_minutes: int = 15):
    ...
    res = supabase.table("security_locks").select("*").eq("username", username).execute()
...
def login_page():
    ...
    if st.button("Login"):
        ...
        if success:
            ...
            st.session_state["session_id"] = create_session(result["username"])
            st.rerun()
        else:
...
def require_login():
    ...
    if st.session_state["logged_in"]:
        if not validate_active_session(timeout_minutes=60):
            st.session_state["logged_in"] = False
            st.session_state["user"] = {}
            st.session_state["session_id"] = None
